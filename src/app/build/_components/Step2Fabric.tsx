...
export default function Step2Fabric() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await supabase
        ...
        .select('fabric_id, fabrics(*)')
        .eq('product_id', state.productId)

      const result = (data ?? [])
        .map((row: any) => row.fabrics)
        .filter((f: any) => f && f.available)

      setFabrics(result)

      // Auto-select if only one option
      if (result.length === 1 && !selectedId) {
  ...
  function getGsmTiers(fabric: Fabric) {
    const min = fabric.gsm_min ?? 160
    const max = fabric.gsm_max ?? 400
    const def = fabric.gsm_default ?? Math.round((min + max) / 2)
    return [
      { label: 'Lightweight', gsm: min, isDefault: false },
      { label: 'Standard', gsm: def, isDefault: true },
      { label: 'Premium', gsm: max, isDefault: false },
    ]
  }
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-3">
        {fabrics.map(fabric => {
          ...
          const tiers = getGsmTiers(fabric)
          return (
            <button
              key={fabric.id}
            ...
            >
              <div className="flex items-start justify-between gap-4 mb-3">
                ...
                {isSelected && (
                  <span className="text-accent text-xs font-medium flex-shrink-0">Selected ✓</span>
                )}
              </div>
              ...
              <div className="flex gap-2 mb-3">
                {tiers.map(tier => (
                  ...
                  >
                    {tier.label} · {tier.gsm}gsm
                  </span>
                ))}
              </div>

              {fabric.description && (
                <p className="text-text-muted text-xs">{fabric.description}</p>
              )}
            </button>
          )
        })}
