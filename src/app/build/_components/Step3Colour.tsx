...
export default function Step3Colour() {
  ...
  function handleContinue() {
    if (!selected) return
    update({
      colourId: selected.id,
      colourName: selected.name,
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap gap-3">
        {Array.from({ length: 10 }).map((_, i) => (
          <div key={i} className="w-11 h-11 rounded-full bg-raised animate-pulse" />
        ))}
      </div>
    </div>
  )
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-text-muted text-sm">All colours are available from MOQ 50 unless noted.</p>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        {colours.map(colour => {
          ...
          return (
            <button
              key={colour.id}
              onClick={() => setSelectedId(colour.id)}
              title={colour.name}
              className="flex flex-col items-center gap-1.5 group"
            >
              <div
                className={`w-11 h-11 rounded-full transition-all
                  ${isSelected ? 'ring-2 ring-white ring-offset-2 ring-offset-bg scale-110' : 'hover:scale-105'}
      ...
      {selected && (
        <div className="bg-raised border border-border rounded-card p-4 flex items-center gap-3">
          <div
            className={`w-8 h-8 rounded-full flex-shrink-0 ${isLightColour(selected.hex) ? 'border border-border' : ''}`}
            style={{ backgroundColor: selected.hex }}
          />
          <div>
            <p className="text-white text-sm font-medium">{selected.name}</p>
            {selected.pantone && (
              <p className="text-text-muted text-xs">Pantone: {selected.pantone}</p>
            )}
          </div>
