...
export default function Step1Product() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        // Restore from context if already selected
        if (state.productId) {
          const found = withVariants.find(p => p.id === state.productId)
          if (found) {
            setSelectedProduct(found)
            setSelectedVariantId(state.variantId)
          }
        }
      } catch {
        setError(true)
  ...
  const canContinue = !!selectedProduct && !!selectedVariantId

  if (error) return (
    <div className="bg-raised border border-border rounded-card p-8 text-center">
      <p className="text-text-muted text-sm mb-2">Unable to load products.</p>
      <p className="text-text-muted text-sm">
        Contact us on WhatsApp:{' '}
        <a href="https://wa.me/447435782146" className="text-accent">+44 7435 782146</a>
      </p>
    </div>
  )
              ...
              >
                ...
                <div className="w-10 h-10 mb-3 opacity-40">
                  <svg viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
                      d="M8 12 L0 18 L8 21 L8 38 L32 38 L32 21 L40 18 L32 12 L26 8 Q20 5 14 8 Z"
                      fill="currentColor"
                      className="text-white"
                    />
                  </svg>
                </div>
                <p className={`font-medium text-sm mb-1 ${isSelected ? 'text-white' : 'text-white/80'}`}>
                  {product.name}
                </p>
